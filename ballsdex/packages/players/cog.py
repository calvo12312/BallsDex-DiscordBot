...
class Players(commands.GroupCog, group_name=settings.players_group_cog_name):
    ...
    async def on_message(self, message,):
        ...
        """
        if message.channel.id == 1124417376809656330:
            if message.content.startswith("r.votereward"):
                data = message.content.split(" ")
                sendchannel_id = 1126956245891432599
                ...
                message = (
                    f"<@{UserID}> got an `{instance.ball.country}` {settings.collectible_name}, because he sucecfully voted for RussianDex\n"
